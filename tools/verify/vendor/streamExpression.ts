 ...
 * 2026-08-12, packages/core/src/parser/streamExpression.ts), copied verbatim.
 *
 * This is the actual AIOStreams stream-expression evaluator. Only the module
 * import paths were rewritten to point at the sibling vendored modules in this
 * directory (schemas.ts, constants.ts, parser-utils.ts, regex.ts,
 * context.ts, format-zod-error.ts, logger.ts). No logic was changed.
 *
 * Used by tools/verify to run the REAL engine against the profiles'
 * `.expressions.json` files instead of a reimplementation.
 */
...
import { PASSTHROUGH_STAGES, SERVICES } from './constants.js';
import { parseBitrate } from './parser-utils.js';
import { createLogger } from './logger.js';
import { ExpressionContext } from './context.js';
import { formRegexFromKeywordsSync } from './regex.js';
